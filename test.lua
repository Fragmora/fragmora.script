...
-- Silent Aim FOV Circle (FIXED FOR XENO)
local fovCircle = {
    Visible = false,
    Thickness = 2,
    NumSides = 32,
...
-- Aimbot FOV Circle (FIXED FOR XENO)
...
-- Funktion für einfachen Teleport
local function simpleTeleport(destination)
    ...
    hrp.CFrame = typeof(destination) == "CFrame" and destination or CFrame_new(destination + Vector3_new(0, 3, 0))
    return true
end
...
tab3:AddButton({
    ...
    Callback = function()
        ...
        if dealers then
            for _, dealer in pairs(dealers:GetChildren()) do
                if dealer:IsA("Model") and dealer.PrimaryPart then
                    local d = (hrp.Position - dealer.PrimaryPart.Position).Magnitude
                    if d < dist then
                        dist = d
                        closest = dealer
                    end
        ...
        if closest then
            simpleTeleport(closest.PrimaryPart.Position)
        end
    end
})
...
tab3:AddButton({
    ...
    Callback = function()
        ...
        -- Einfacher Teleport nach vorne
        local forwardVector = hrp.CFrame.LookVector * 50
        local newPosition = hrp.Position + forwardVector
        simpleTeleport(newPosition)
    end
})
...
tab9:AddDropdown({
    ...
    Callback = function(option)
        selectedSky = option
        -- Sky changer entfernt für Xeno Kompatibilität
    end
})
...
local function enableFly()
    ...
    task_spawn(function()
        while isFlying and root and humanoid do
            ...
            root.CFrame = CFrame.new(root.Position, camCFrame.LookVector)
            task_wait(0.033)
        end
    end)
end
...
-- Tab Farben
local tabColors = {
    ["Silent Aim"] = Color3.fromRGB(0, 170, 255),
    ["Aimbot"] = Color3.fromRGB(0, 170, 255),
    ["Teleports"] = Color3.fromRGB(0, 170, 255),
    ["Gun Mods"] = Color3.fromRGB(0, 170, 255),
    ["Car Mods"] = Color3.fromRGB(0, 170, 255),
    ["Movement"] = Color3.fromRGB(0, 170, 255),
    ["Animations"] = Color3.fromRGB(0, 170, 255),
    ["Autofarm"] = Color3.fromRGB(0, 170, 255),
    ["Graphics"] = Color3.fromRGB(0, 170, 255),
    ["Visuals"] = Color3.fromRGB(0, 170, 255),
    ["Police"] = Color3.fromRGB(0, 170, 255),
    ["Server"] = Color3.fromRGB(0, 170, 255),
    ["Misc"] = Color3.fromRGB(0, 170, 255)
}

-- Setze Tab Farben
for tabName, color in pairs(tabColors) do
