...
local function setSilentAimKeybind()
    silentAimKeybindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        silentAimKeybind = input.KeyCode
        local keyName = tostring(silentAimKeybind):gsub("Enum.KeyCode.", "")
        silentAimKeybindText:Set("Silent Aim Key: " .. keyName)
    else
        silentAimKeybindText:Set("Silent Aim Key: E")
    end
end
...
local function setHoldToShootKeybind()
    holdToShootText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
...
local function setAimbotKeybind()
    aimbotKeybindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        aimbotKeybind = input.KeyCode
        local keyName = tostring(aimbotKeybind):gsub("Enum.KeyCode.", "")
        aimbotKeybindText:Set("Aimbot Key: " .. keyName)
    else
        aimbotKeybindText:Set("Aimbot Key: Q")
    end
end
...
local function updatePlayerListForWhitelist()
    ...
    if #playerListForWhitelist == 0 then
        table_insert(playerListForWhitelist, "No players")
    end
end
    ...
    Callback = function()
        if selectedWhitelistPlayer then
            ...
            OrionLib:MakeNotification({
                Name = "Whitelist",
                Content = "Added " .. selectedWhitelistPlayer.Name .. " to whitelist",
...
tab2:AddButton({
    ...
    Callback = function()
        if selectedWhitelistPlayer then
            ...
            OrionLib:MakeNotification({
                Name = "Whitelist",
                Content = "Removed " .. selectedWhitelistPlayer.Name .. " from whitelist",
                Image = "rbxassetid://7733674079",
                Time = 3
            })
        end
...
local function setTriggerbotKeybind()
    triggerbotKeybindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        triggerbotKeybind = input.KeyCode
        local keyName = tostring(triggerbotKeybind):gsub("Enum.KeyCode.", "")
        triggerbotKeybindText:Set("Triggerbot Key: " .. keyName)
    else
        triggerbotKeybindText:Set("Triggerbot Key: T")
    end
end
...
task_spawn(function()
    while true do
        updatePlayerListForWhitelist()
        if whitelistPlayerDropdown then
...
local function tweenTo(destination, isVehicle)
    if isVehicle then
        ...
        if not car.PrimaryPart then
            OrionLib:MakeNotification({
                Name = "Teleport",
                Content = "Vehicle has no primary part",
                Image = "rbxassetid://7733674079",
                Time = 3
            })
            return false
        ...
        if char then
            ...
            if hrp then
                local targetPosition = typeof(destination) == "CFrame" and destination.Position or destination
                char:MoveTo(targetPosition + Vector3_new(0, 3, 0))
                
                OrionLib:MakeNotification({
                    Name = "Teleport",
                    Content = "Character teleported",
                    Image = "rbxassetid://7733674079",
                    Time = 3
                })
                return true
        ...
        if dealers then
            for _, dealer in pairs(dealers:GetChildren()) do
                if dealer:IsA("Model") and dealer.PrimaryPart then
                    local d = (char.HumanoidRootPart.Position - dealer.PrimaryPart.Position).Magnitude
                    if d < dist then
                        dist = d
                        closest = dealer
                    end
...
tab3:AddButton({
    ...
    Callback = function()
        ...
        if closest then
            ...
            OrionLib:MakeNotification({
                Name = "Teleport",
                Content = "No vending machine found",
                Image = "rbxassetid://7733674079",
                Time = 3
            })
        end
...
tab4:AddSlider({
    ...
    Callback = function(v)
        crosshairSize = v
        -- Hier Crosshair Update implementieren
    end
})
...
local function setCarFlyKeybind()
    carFlyKeybindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        carFlyKeybind = input.KeyCode
        local keyName = tostring(carFlyKeybind):gsub("Enum.KeyCode.", "")
        carFlyKeybindText:Set("Car Fly Key: " .. keyName)
    else
        carFlyKeybindText:Set("Car Fly Key: X")
    end
end
...
local function setSpeedKeybind()
    speedKeybindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        speedKeybind = input.KeyCode
        local keyName = tostring(speedKeybind):gsub("Enum.KeyCode.", "")
        speedKeybindText:Set("Speed Key: " .. keyName)
    else
        speedKeybindText:Set("Speed Key: Left Shift")
    end
end
...
tab6:AddButton({
    ...
    Callback = function()
        ...
        if closestBike then
            ...
            if driveSeat and LocalPlayer.Character:FindFirstChild("Humanoid") then
                driveSeat:Sit(LocalPlayer.Character.Humanoid)
            end
        end
    end
...
local function setAnimationKeybind()
    animationKeybindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        animationToggleKey = input.KeyCode
        local keyName = tostring(animationToggleKey):gsub("Enum.KeyCode.", "")
        animationKeybindText:Set("Animation Key: " .. keyName)
    else
        animationKeybindText:Set("Animation Key: Z")
    end
end
...
local function changeSky(skyName)
    ...
    if skyName ~= "Default" then
        ...
        elseif skyName == "Midnight" then
            -- Midnight Sky
            lighting.ClockTime = 0
            lighting.OutdoorAmbient = Color3.fromRGB(50, 50, 100)
        elseif skyName == "Purple" then
            lighting.FogColor = Color3.fromRGB(100, 0, 100)
            lighting.OutdoorAmbient = Color3.fromRGB(100, 0, 100)
        elseif skyName == "Blue" then
            lighting.FogColor = Color3.fromRGB(0, 100, 200)
            lighting.OutdoorAmbient = Color3.fromRGB(0, 100, 200)
        end
    end
end
...
local function updateSkinChanger()
    if skinChanger and LocalPlayer.Character then
        ...
        OrionLib:MakeNotification({
            Name = "Graphics",
            Content = "Skin changer enabled (simulated)",
            Image = "rbxassetid://7733674079",
            Time = 2
        })
    end
...
local function updateTrails()
    if trails and LocalPlayer.Character then
        if not trailAttachment then
            local hrp = LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
            if hrp then
                trailAttachment = Instance.new("Trail")
                trailAttachment.Attachment0 = Instance.new("Attachment", hrp)
                trailAttachment.Attachment1 = Instance.new("Attachment", hrp)
...
local function fireTaserAtTarget()
    ...
    local target = findNearestEnemy(80) -- Max Tase Range
    ...
    OrionLib:MakeNotification({
        Name = "Auto Taser",
        Content = "Firing taser at " .. target.Name,
        Image = "rbxassetid://7733674079",
        Time = 2
    })
end
            ...
            if player ~= LocalPlayer and player.Team ~= LocalPlayer.Team then
                ...
                if targetHrp then
                    ...
                    if distance <= cuffDistance then
                        OrionLib:MakeNotification({
                            Name = "Auto Cuff",
                            Content = "Attempting to cuff: " .. player.Name,
                            Image = "rbxassetid://7733674079",
                            Time = 2
                        })
                    end
...
-- Police Update Loops
RunService.RenderStepped:Connect(function()
    if autoTaser and tick() - lastTase >= 0.5 then
        fireTaserAtTarget()
        lastTase = tick()
    end
end)
...
tab11:AddToggle({
    ...
    Callback = function(v)
        ...
        OrionLib:MakeNotification({
            Name = "Police Tools",
            Content = "Auto stop stick: " .. (v and "ON" or "OFF"),
            Image = "rbxassetid://7733674079",
            Time = 2
        })
    end
...
local function antiArrestFunction()
    if antiArrest and LocalPlayer.Character then
        ...
        if hrp and hrp:GetAttribute("Arrested") then
...
local function setFlyBind()
    flyBindText:Set("Press a key...")
    local input = UIS.InputBegan:Wait()
    if input.UserInputType == Enum.UserInputType.Keyboard then
        flyBind = input.KeyCode
        local keyName = tostring(flyBind):gsub("Enum.KeyCode.", "")
        flyBindText:Set("Fly Bind: " .. keyName)
    else
        flyBindText:Set("Fly Bind: V")
    end
end
