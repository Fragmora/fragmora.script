...
local function getBestTarget()
    ...
    local function isEnemy(player)
        ...
        if LocalPlayerTeam and LocalPlayerTeam.Name == "Citizen" then
            return player.Team.Name == "Police"
        elseif LocalPlayerTeam and LocalPlayerTeam.Name == "Police" then
...
-- Flashlight Color
local flashlightColor = Color3.fromRGB(255, 255, 255)
local flashlightRainbow = false
...
GunModsTab:AddColorpicker({
    Name = "Flashlight Color",
    Default = flashlightColor,
    Callback = function(color)
        flashlightColor = color
        flashlightRainbow = false
    end
})
...
GunModsTab:AddToggle({
    ...
    Default = false,
    Callback = function(val)
        flashlightRainbow = val
        if val then
            task.spawn(function()
                while flashlightRainbow do
                    local hue = (tick() % 5) / 5
...
local function canSelfRevive()
    if not LocalPlayer.Character then
        return false
    end
    local humanoid = LocalPlayer.Character:FindFirstChildWhichIsA("Humanoid")
    return not humanoid or humanoid.Health < 30
end
...
local function enterHospitalBed()
    ...
    local seat = hospitalBed and hospitalBed:FindFirstChild("Seat")
    local character = LocalPlayer.Character
...
local function selfReviveSequence()
    if canSelfRevive() then
        ...
        game.StarterGui:SetCore("SendNotification", {
            Title = "Self Revive",
            Text = "You are not dead, you can't perform a self revive!",
            Duration = 5
        })
    end
end
    ...
    if bringCarToMe() then
        ...
        if vehicle and character then
            ...
            if humanoid and driveSeat then
                driveSeat:Sit(humanoid)
                task.wait(0.5)
                tweenObjectToPosition(vehicle, location.Position, teleportSpeed)
            end
        end
    end
end
...
ESPTab:AddSlider({
    ...
    Min = 1,
    Max = 6,
    Default = GetSetting("SkeletonThickness", 3),
    Increment = 1,
    ValueName = "px",
    Color = Color3.fromRGB(255,255,255),
    Callback = function(Value)
        ...
        if val then 
            task.spawn(function() 
                while infiniteFuelEnabled do 
                    local car = workspace.Vehicles:FindFirstChild(LocalPlayer.Name) 
                    if car then 
                        car:SetAttribute("currentFuel", 9999999999999999999999) 
                    end
                    task.wait(0.01) 
                end
            end)
...
VehicleModsTab2:AddDropdown({
    ...
    Callback = function(val)
        if val == "City Loop - Start at Jeweler Parking" then
            selectedRoute = "City Loop"
        else
            selectedRoute = "Highway Run"
        end
        OrionLib:MakeNotification({
            Name = "Route Selected",
...
local function startAutoTaser()
    ...
    autoTaserLoop = RunService.Heartbeat:Connect(function()
        ...
        local predictedPosition = targetPosition + (targetVelocity * AutoTaserSettings.prediction)
        local aimDirection = (predictedPosition - root.Position).Unit
...
local radarRemote = ReplicatedStorage:FindFirstChild("WvO"):FindFirstChild("e3a0cb13-da43-46b9-ba5d-e6ea0d913417")
...
local function scanAndProcessVehicles(character, radarTool)
    ...
    for _, vehicle in ipairs(vehicles) do
        ...
        radarRemote:FireServer(radarTool, targetPosition, direction)
    end
end
    ...
    while RadarFarm.Enabled do
        ...
        if currentTime - lastFireTime >= RadarFarm.FireRate then
            ...
            if character then
                ...
                if radarTool and radarRemote then
...
local function populateDanceDropdown()
    ...
    danceDropdown:Refresh(danceNames, danceNames[1] or "")
end
...
AnimTab:AddButton({
    ...
    Callback = function()
        local selected = danceDropdown.Value
        local animId = getAnimationIdByName(selected)
